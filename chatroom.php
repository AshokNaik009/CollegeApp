       ...
       if(isset($_POST['submit']))
       {
           ...
           $run=$db1->query($insert);
            ...
            if($insert)
            {
                ...
                $sec=1;
                 header("Refresh:$sec;url='chatroom.php?id=$rec'");
               ?>
               <script>
                  alert("Message sent");
...
<meta http-equiv="refresh"  content="1:url=http://localhost/collegeApp/chatroom.php?id=$_GET['id'];"/>
</head>
...
<div class="col-lg-2   customDiv drop-shadow" style="width:20%;background-color:white;font-size:14px;" >
   <?php
    $rec=$_GET['id'];
...
<div class="col-lg-6"  style="background-color:white;margin-left:15px" >
...
<div class="chatbox" style='position:fixed;border:1px solid;margin-left:245px;margin-top:1%;padding:5px; width:38%;left:12%;height:120px;' >
<div  >
...
<textarea name="msg"  rows='5'  cols='30'     style='resize:none;width:40%; ' ></textarea><br/>
             <button class="btn btn-success pull-left" style="width:15%;margin-left:65%;margin-top:-35px" name="submit"><span class="glyphicon glyphicon-send"> </span> &nbsp; Send</button>
</form>
</div>
</div>
</div>
...
<div class="col-lg-2 customDiv  drop-shadow " style=" width:20%">
<h2>Friends List</h2>
<?php
...
while($rows=mysql_fetch_assoc($query))
{
  $f1=$rows['user_two'];
     ...
     while($res=mysql_fetch_assoc($query1))
     {
      ...
      $p2=$res['stu_pic'];
        if($p2!=""){ echo "<br/><img src='profile/$p2' width='50px' height='50px' style='border-radius:50%;'><br/>";}
           echo $res['userstud']."<br/><a href='chatroom.php?id={$res['userstud']}'><button class='btn btn-primary' type='submit'>  Message &nbsp;<span class='glyphicon glyphicon-send'> </span></button></a>";
   ...
   while($res=mysql_fetch_assoc($query1))
   {
    ...
    $p2=$res['stu_pic'];
      if($p2!=""){ echo "<br/><img src='profile/$p2' width='50px' height='50px' style='border-radius:50%;'><br/>";}
      echo $res['userstud'] ."<br/><a href='chatroom.php?id={$res['userstud']}'><button class='btn btn-primary' type='submit'> <span class='glyphicon glyphicon-send'> </span> &nbsp;Message</button></a>";
